 ...
 */
function sanitizeStreamerName(name) {
  if (!name || typeof name !== 'string') {
    throw new Error('Invalid streamer name');
  }
  
  // Remove any potentially dangerous characters and encode properly
  ...
  if (!sanitized || sanitized.length === 0) {
    throw new Error('Streamer name is empty after sanitization');
  }
  
  // Additional validation: only allow alphanumeric, dots, underscores, and hyphens
  if (!/^[a-zA-Z0-9._-]+$/.test(sanitized)) {
    throw new Error('Streamer name contains invalid characters');
  }
...
async function checkYouTubeLive(streamer) {
  try {
    ...
    // Sanitize the streamer name to prevent URL injection
    const sanitizedName = sanitizeStreamerName(streamer.name);
    const safeUrl = `https://www.youtube.com/@${sanitizedName}`;
  ...
  thumbnail: "${thumbnailUrl || 'None'}"
    ...
    try {
      const sanitizedName = sanitizeStreamerName(streamer.name);
      fallbackUrl = `https://www.youtube.com/@${sanitizedName}`;
    } catch (sanitizeError) {
