...
const { LiveConnector, getRoomIdFromMainPageUrl } = require('tiktok-live-connector');
...
async function checkTikTokLive(streamer) {
  ...
  try {
    ...
    // Construct safe URLs
    const safeUrl = constructSafeUrl('tiktok', streamer.name);
  ...
  thumbnail: "${thumbnailUrl || 'None'}"
    ...
    try {
      fallbackUrl = constructSafeUrl('tiktok', streamer.name);
    } catch (sanitizeError) {
