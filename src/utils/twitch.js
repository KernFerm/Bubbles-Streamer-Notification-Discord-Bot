...
async function checkTwitchLive(streamer) {
  try {
    ...
    // Construct safe URL once for reuse
    const safeUrl = constructSafeUrl('twitch', streamer.name);
    ...
    try {
      const safeApiUrl = constructSafeApiUrl('twitch', streamer.name, 'streams');
      const apiResponse = await fetch(safeApiUrl, {
        headers: {
    ...
    try {
      fallbackUrl = constructSafeUrl('twitch', streamer.name);
    } catch (sanitizeError) {
