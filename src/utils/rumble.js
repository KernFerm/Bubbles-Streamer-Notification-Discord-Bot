// src/utils/rumble.js
const { constructSafeApiUrl, constructSafeUrl } = require('./urlSanitizer');
...
async function checkRumbleLive(streamer) {
  try {
    ...
    // Construct safe URLs
    const safeApiUrl = constructSafeApiUrl('rumble', streamer.name, 'user');
    const safeUrl = constructSafeUrl('rumble', streamer.name);
    
    console.log(`Fetching data from ${safeApiUrl}...`);
    ...
    console.log(`Successfully loaded URL: ${safeApiUrl}`);
  ...
  thumbnail: "${thumbnailUrl || 'None'}"
    ...
    try {
      fallbackUrl = constructSafeUrl('rumble', streamer.name);
    } catch (sanitizeError) {
